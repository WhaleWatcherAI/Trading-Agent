#!/usr/bin/env tsx
/**
 * Engulfing + Three Soldiers Strategy Backtest
 *
 * Strategy:
 * - Entry signals (both must occur within 4 bars of each other, in any order):
 *   1. Engulfing Pattern: Bullish engulfing (for long) or Bearish engulfing (for short)
 *   2. Three Soldiers: 3 consecutive bullish candles (for long) or 3 consecutive bearish candles (for short)
 *
 * - Exit signals (either triggers exit):
 *   1. Opposite engulfing pattern
 *   2. Three opposite soldiers (3 consecutive candles in opposite direction)
 *   3. MACD inverse crossover (optional)
 *
 * - Parameters:
 *   - Soldiers: 3 consecutive candles of same color
 *   - Signal window: Both signals must occur within 4 bars
 *   - No stop loss (let signals dictate exits)
 */
...
interface TradeRecord {
  ...
  grossPnl: number;
  fees: number;
  exitReason: 'macd_cross' | 'opposite_soldiers' | 'opposite_engulfing' | 'stop' | 'target' | 'session' | 'end_of_data';
  entrySignal: string;
}
...
const DEFAULT_SYMBOL = process.env.ENGULF_SOLDIERS_SYMBOL || 'NQZ5';
const DEFAULT_CONTRACT_ID = process.env.ENGULF_SOLDIERS_CONTRACT_ID;
...
const CONFIG: BacktestConfig = {
  ...
  start: process.env.ENGULF_SOLDIERS_START || new Date(Date.now() - DEFAULT_DAYS * 24 * 60 * 60 * 1000).toISOString(),
  end: process.env.ENGULF_SOLDIERS_END || new Date().toISOString(),
  soldiersCount: Number(process.env.ENGULF_SOLDIERS_COUNT || '3'),
  signalWindowBars: Number(process.env.ENGULF_SOLDIERS_WINDOW || '4'),
  numberOfContracts: Number(process.env.ENGULF_SOLDIERS_CONTRACTS || '1'),
  commissionPerSide: process.env.ENGULF_SOLDIERS_COMMISSION
    ? Number(process.env.ENGULF_SOLDIERS_COMMISSION)
    : inferFuturesCommissionPerSide([DEFAULT_CONTRACT_ID, DEFAULT_SYMBOL], 0.35),
  slippageTicks: Number(process.env.ENGULF_SOLDIERS_SLIPPAGE_TICKS || '1'),
  useMacdExit: process.env.ENGULF_SOLDIERS_USE_MACD_EXIT === 'true',
  macdFastPeriod: Number(process.env.ENGULF_SOLDIERS_MACD_FAST || '12'),
  macdSlowPeriod: Number(process.env.ENGULF_SOLDIERS_MACD_SLOW || '26'),
  macdSignalPeriod: Number(process.env.ENGULF_SOLDIERS_MACD_SIGNAL || '9'),
  stopLossTicks: Number(process.env.ENGULF_SOLDIERS_STOP_TICKS || '8'),
  takeProfitTicks: Number(process.env.ENGULF_SOLDIERS_TP_TICKS || '24'),
};
...
async function runBacktest() {
  console.log('\n' + '='.repeat(80));
  console.log('ENGULFING + THREE SOLDIERS STRATEGY BACKTEST');
  ...
  console.log(`MACD Exit: ${CONFIG.useMacdExit ? 'ENABLED' : 'DISABLED'}`);
  if (CONFIG.useMacdExit) {
    console.log(`MACD: ${CONFIG.macdFastPeriod}, ${CONFIG.macdSlowPeriod}, ${CONFIG.macdSignalPeriod}`);
  }
  console.log(`No Stop Loss | Contracts: ${CONFIG.numberOfContracts}`);
  ...
  const metadata = await fetchTopstepXFuturesMetadata(lookupKey).catch(err => {
    console.warn('[engulfing-soldiers] Unable to fetch metadata:', err.message);
    return null;
  });
  ...
  for (let i = 1; i < bars.length; i++) {
    ...
    let currentMACD: { MACD: number; signal: number; histogram: number } | null = null;
    let prevMACD: { MACD: number; signal: number; histogram: number } | null = null;
    let macdCross = { bullishCross: false, bearishCross: false };

    if (CONFIG.useMacdExit && closes.length >= CONFIG.macdSlowPeriod + CONFIG.macdSignalPeriod) {
      ...
      currentMACD = macdValues.length >= 2 ? macdValues[macdValues.length - 1] : null;
      prevMACD = macdValues.length >= 2 ? macdValues[macdValues.length - 2] : null;

      if (currentMACD && prevMACD) {
        macdCross = detectMacdCrossover(currentMACD, prevMACD);
      }
    }

    // Detect engulfing patterns
    const bullishEngulfing = isBullishEngulfing(bar, prevBar);
    const bearishEngulfing = isBearishEngulfing(bar, prevBar);
    ...
    if (position) {
      // Exit logic: stop loss, target, opposite engulfing, three opposite soldiers, OR inverse MACD (if enabled)
      ...
      // Take profit disabled - let signals dictate exits
      ...
      //   const targetExitPrice = roundToTick(fillEntry(baseSymbol, closeSide, targetPrice));
      //   console.log(`[${bar.timestamp}] TARGET hit ${position.side.toUpperCase()} @ ${targetExitPrice.toFixed(2)}`);
      //   exitPosition(targetExitPrice, bar.timestamp, 'target');
      //   continue;
      // }

      // Check for opposite engulfing
      const oppositeEngulfing = (position.side === 'long' && bearishEngulfing) ||
                                (position.side === 'short' && bullishEngulfing);
      ...
      // Check for inverse MACD crossover (if enabled)
      const inverseMacdCross = CONFIG.useMacdExit && (
        (position.side === 'long' && macdCross.bearishCross) ||
        (position.side === 'short' && macdCross.bullishCross)
      );

      if (oppositeEngulfing) {
        const closeSide = position.side === 'long' ? 'sell' : 'buy';
        const exitPrice = roundToTick(fillEntry(baseSymbol, closeSide, bar.close));
        console.log(`[${bar.timestamp}] EXIT ${position.side.toUpperCase()} @ ${exitPrice.toFixed(2)} - Opposite engulfing pattern`);
        exitPosition(exitPrice, bar.timestamp, 'opposite_engulfing');
        continue;
      }
    ...
    if (bullishEngulfing) {
      if (pendingSignal?.type === 'soldiers' && pendingSignal.side === 'long') {
        ...
        position = {
          ...
          entrySignal: `Bullish Engulfing + 3 Soldiers (${i - pendingSignal.barIndex} bars apart)`,
          stopLoss: roundToTick(entryPrice - (CONFIG.stopLossTicks * tickSize)),
          target: roundToTick(entryPrice + (CONFIG.takeProfitTicks * tickSize)),
        };

        console.log(`[${bar.timestamp}] LONG entry @ ${entryPrice.toFixed(2)} - Bullish engulfing + Three soldiers (${i - pendingSignal.barIndex} bars apart)`);
        pendingSignal = null;
      } else if (!pendingSignal || pendingSignal.side !== 'long') {
        // New bullish engulfing signal
        pendingSignal = {
          type: 'engulfing',
          side: 'long',
          barIndex: i,
        ...
        console.log(`[${bar.timestamp}] Bullish engulfing detected @ ${bar.close.toFixed(2)} - awaiting three soldiers within ${CONFIG.signalWindowBars} bars`);
      }
      ...
      if (pendingSignal?.type === 'soldiers' && pendingSignal.side === 'short') {
        ...
        position = {
          ...
          entryPrice,
          entryTime: bar.timestamp,
          entrySignal: `Bearish Engulfing + 3 Soldiers (${i - pendingSignal.barIndex} bars apart)`,
          stopLoss: roundToTick(entryPrice + (CONFIG.stopLossTicks * tickSize)),
          target: roundToTick(entryPrice - (CONFIG.takeProfitTicks * tickSize)),
        };

        console.log(`[${bar.timestamp}] SHORT entry @ ${entryPrice.toFixed(2)} - Bearish engulfing + Three soldiers (${i - pendingSignal.barIndex} bars apart)`);
        pendingSignal = null;
      } else if (!pendingSignal || pendingSignal.side !== 'short') {
        // New bearish engulfing signal
        pendingSignal = {
          type: 'engulfing',
          side: 'short',
          barIndex: i,
        ...
        console.log(`[${bar.timestamp}] Bearish engulfing detected @ ${bar.close.toFixed(2)} - awaiting three soldiers within ${CONFIG.signalWindowBars} bars`);
      }
    }
    ...
    if (soldiers) {
      if (soldiers.bullish) {
        if (pendingSignal?.type === 'engulfing' && pendingSignal.side === 'long') {
          // Both signals detected - enter long
          const entrySide = 'buy';
          const entryPrice = roundToTick(fillEntry(baseSymbol, entrySide, bar.close));
          ...
          position = {
            ...
            entryPrice,
            entryTime: bar.timestamp,
            entrySignal: `3 Soldiers + Bullish Engulfing (${i - pendingSignal.barIndex} bars apart)`,
          stopLoss: roundToTick(entryPrice - (CONFIG.stopLossTicks * tickSize)),
          target: roundToTick(entryPrice + (CONFIG.takeProfitTicks * tickSize)),
        };

          console.log(`[${bar.timestamp}] LONG entry @ ${entryPrice.toFixed(2)} - Three soldiers + Bullish engulfing (${i - pendingSignal.barIndex} bars apart)`);
          pendingSignal = null;
        } else if (!pendingSignal || pendingSignal.side !== 'long') {
          ...
          console.log(`[${bar.timestamp}] Three bullish soldiers detected @ ${bar.close.toFixed(2)} - awaiting engulfing within ${CONFIG.signalWindowBars} bars`);
        }
      } else if (soldiers.bearish) {
        if (pendingSignal?.type === 'engulfing' && pendingSignal.side === 'short') {
          // Both signals detected - enter short
          const entrySide = 'sell';
          const entryPrice = roundToTick(fillEntry(baseSymbol, entrySide, bar.close));
          ...
          position = {
            ...
            entryPrice,
            entryTime: bar.timestamp,
            entrySignal: `3 Soldiers + Bearish Engulfing (${i - pendingSignal.barIndex} bars apart)`,
          stopLoss: roundToTick(entryPrice + (CONFIG.stopLossTicks * tickSize)),
          target: roundToTick(entryPrice - (CONFIG.takeProfitTicks * tickSize)),
        };

          console.log(`[${bar.timestamp}] SHORT entry @ ${entryPrice.toFixed(2)} - Three soldiers + Bearish engulfing (${i - pendingSignal.barIndex} bars apart)`);
          pendingSignal = null;
        } else if (!pendingSignal || pendingSignal.side !== 'short') {
          ...
          console.log(`[${bar.timestamp}] Three bearish soldiers detected @ ${bar.close.toFixed(2)} - awaiting engulfing within ${CONFIG.signalWindowBars} bars`);
        }
      }
    }
  }
...
runBacktest().catch(err => {
  console.error('Engulfing + Three Soldiers backtest failed:', err);
  process.exit(1);
});
