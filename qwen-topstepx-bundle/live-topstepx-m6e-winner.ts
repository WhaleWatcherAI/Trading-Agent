#!/usr/bin/env tsx
/**
 * TopstepX Live M6E Winner Strategy - ENHANCED VERSION
 *
 * Features:
...
const CONFIG: StrategyConfig = {
  symbol: process.env.TOPSTEPX_NQ_LIVE_SYMBOL || '6EZ5',
...
const DASHBOARD_PORT = Number(process.env.TOPSTEPX_NQ_DASHBOARD_PORT || '3336');
...
const TRADE_LOG_FILE = process.env.TOPSTEPX_NQ_TRADE_LOG || './logs/topstepx-m6e-winner.jsonl';
const TRADE_LOG_DIR = path.dirname(TRADE_LOG_FILE);
const STATE_FILE = './logs/.m6e-state.json';
...
async function enterPosition(
  ...
  const targetTicks = side === 'long' ? 8 : -8; // Positive for LONG, negative for SHORT
...
async function main() {
  console.log('\n' + '='.repeat(80));
  console.log('TOPSTEPX LIVE M6E WINNER STRATEGY - ENHANCED VERSION');
