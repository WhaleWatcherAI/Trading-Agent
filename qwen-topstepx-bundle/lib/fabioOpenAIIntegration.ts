...
/**
 * Calculate CVD momentum from 1-minute CVD candles using 20 EMA of delta
 * Delta = close - open for each candle (net volume change in that minute)
 */
function computeCvdMomentum(cvdCandles: CurrentCvdBar[]): {
  deltaEma20: number;
  trend: 'up' | 'down';  // No neutral - like ATAS, always bullish or bearish
  trendStrength: 'strong' | 'moderate' | 'weak';
  minutesSinceTrendSwitch: number | null;
  trendSwitchContext: string | null;
} | null {
  // Need at least 5 candles to compute meaningful EMA
  if (!cvdCandles || cvdCandles.length < 5) {
    return null;
  }

  // Extract delta (close - open) from each candle
  const deltas = cvdCandles.map(c => c.close - c.open);

  // Calculate 20-period EMA of deltas
  // EMA formula: EMA = price * k + EMA_prev * (1 - k), where k = 2/(period+1)
  const period = Math.min(20, deltas.length);
  const k = 2 / (period + 1);

  // Initialize EMA with SMA of first `period` values (or all if less)
  const initPeriod = Math.min(period, deltas.length);
  let ema = deltas.slice(0, initPeriod).reduce((sum, d) => sum + d, 0) / initPeriod;

  // Calculate EMA for remaining values
  for (let i = initPeriod; i < deltas.length; i++) {
    ema = deltas[i] * k + ema * (1 - k);
  }

  // Determine trend direction based on EMA sign (like ATAS - no neutral, only bullish/bearish)
  // If EMA >= 0, buyers are winning (up/bullish)
  // If EMA < 0, sellers are winning (down/bearish)
  const trend: 'up' | 'down' = ema >= 0 ? 'up' : 'down';

  // Calculate trend strength based on EMA magnitude
  const absDelta = Math.abs(ema);
  ...
  // Track when trend switched by looking at historical EMA values
  // Calculate EMA at each point going back to find switch
  let minutesSinceTrendSwitch: number | null = null;
  let trendSwitchContext: string | null = null;

  if (deltas.length >= 3) {
    // Recalculate EMA at each historical point to find when sign changed
    const historicalEmas: number[] = [];
    const minLen = Math.min(period, deltas.length);
    let runningEma = deltas.slice(0, minLen).reduce((sum, d) => sum + d, 0) / minLen;
    historicalEmas.push(runningEma);

    for (let i = minLen; i < deltas.length; i++) {
      runningEma = deltas[i] * k + runningEma * (1 - k);
      historicalEmas.push(runningEma);
    }

    // Find the most recent sign change (from positive to negative or vice versa)
    // No neutral - just positive (>=0) or negative (<0) like ATAS
    const currentSign = ema >= 0 ? 'positive' : 'negative';

    for (let i = historicalEmas.length - 2; i >= 0; i--) {
      const pastEma = historicalEmas[i];
      const pastSign = pastEma >= 0 ? 'positive' : 'negative';

      if (pastSign !== currentSign) {
        // Found a sign change
        minutesSinceTrendSwitch = (historicalEmas.length - 1) - i;

        // Only report if within 3 minutes
        if (minutesSinceTrendSwitch <= 3) {
          if (minutesSinceTrendSwitch === 0 || minutesSinceTrendSwitch === 1) {
            trendSwitchContext = 'trend just switched';
          } else {
            trendSwitchContext = `trend switched ${minutesSinceTrendSwitch} minute${minutesSinceTrendSwitch > 1 ? 's' : ''} ago`;
          }
        } else {
          minutesSinceTrendSwitch = null; // Don't report if older than 3 minutes
        }
        break;
      }
    }
  ...
  return {
    deltaEma20: Math.round(ema * 100) / 100, // Round to 2 decimal places
    trend,
    trendStrength,
    minutesSinceTrendSwitch,
    trendSwitchContext,
  };
}
...
export function buildFuturesMarketData(
  ...
  footprintCandles: FuturesMarketData['footprintCandles'] = [],
): FuturesMarketData {
  ...
  openPosition: currentPosition
    ? {
        ...
        positionVersion: currentPosition?.positionVersion,
      }
    : null,
    ...
    marketStats,
    cvdCandles,
    cvdMomentum: computeCvdMomentum(cvdCandles) ?? undefined,
    performance,
    historicalNotes,
    microstructure,
    macrostructure,
    ...
    connectionHealth,
    // Prefer live-tracked footprint candles from playbook; fall back to legacy builder
    footprintCandles: footprintCandles.length > 0 ? footprintCandles : buildFootprintCandles(orderFlowData, tickSize ?? 0.25),
    brokenWalls: detectBrokenWalls(microstructure, currentPrice, tickSize ?? 0.25),
  };
...
/**
 * Aggregate bars into higher timeframes (5m, 30m, 60m)
 */
function aggregateBarsToTimeframe(
  ...
  for (const bar of bars) {
    ...
    const bucketKey = Math.floor(ts / periodMs) * periodMs;
...
function buildMacrostructureFromBars(
  ...
  // Build 5m candles from 1m bars
  const fiveMinCandles = aggregateBarsToTimeframe(bars, 5 * 60 * 1000);
  if (fiveMinCandles.length > 0) {
    higherTimeframes.push({
      timeframe: '5m',
      candles: fiveMinCandles.slice(-30), // last 30 x 5m = 2.5 hours
    });
  }

  // Build 30m candles from 1m bars
  const thirtyMinCandles = aggregateBarsToTimeframe(bars, 30 * 60 * 1000);
  if (thirtyMinCandles.length > 0) {
    higherTimeframes.push({
      timeframe: '30m',
      candles: thirtyMinCandles.slice(-20), // last 20 x 30m = 10 hours
    });
  }

  // Build 60m candles from 1m bars
  const hourlyCandles = aggregateBarsToTimeframe(bars, 60 * 60 * 1000);
  if (hourlyCandles.length > 0) {
    higherTimeframes.push({
      timeframe: '60m',
      candles: hourlyCandles.slice(-12), // last 12 hours
    });
  }

  // Add externally provided higher timeframes (4h, 1d from API)
...
// ------------------- Footprint & Broken Walls -------------------

/**
 * Build footprint candles from order flow data
 * Shows buyer/seller volume at each price level within 5-minute windows
 */
function buildFootprintCandles(
  orderFlowData: OrderFlowData,
  tickSize: number
): FuturesMarketData['footprintCandles'] {
  const now = Date.now();
  const fiveMinMs = 5 * 60 * 1000;
  const bucketStart = Math.floor(now / fiveMinMs) * fiveMinMs;

  // Group volume at price by 5-minute buckets
  const volumeAtPrice = orderFlowData.volumeAtPrice || {};
  const levels: Array<{
    price: number;
    buyVolume: number;
    sellVolume: number;
  ...
  for (const [priceStr, vol] of Object.entries(volumeAtPrice)) {
    const price = Number(priceStr);
    if (Number.isNaN(price)) continue;

    const buyVol = Number((vol as any)?.buy ?? 0);
    const sellVol = Number((vol as any)?.sell ?? 0);
    ...
    if (total > 0) {
      ...
      if (ratio >= 0.7) imbalance = 'buy';
      else if (ratio <= 0.3) imbalance = 'sell';
    }
  ...
  // Sort by total volume (most significant levels first)
  levels.sort((a, b) => (b.buyVolume + b.sellVolume) - (a.buyVolume + a.sellVolume));

  if (levels.length === 0) return undefined;

  return [{
    timestamp: new Date(bucketStart).toISOString(),
    levels: levels.slice(0, 10), // Top 10 levels by volume
  }];
}

// Global state for tracking walls (persists across calls)
const wallHistory: Map<string, { price: number; side: 'bid' | 'ask'; size: number; firstSeen: number; lastSeen: number }> = new Map();
const brokenWallsHistory: Array<{ price: number; side: 'bid' | 'ask'; size: number; brokenAt: string; direction: 'up' | 'down' }> = [];

/**
 * Detect broken walls by comparing current L2 book with history
 * A wall is "broken" if it was significant and no longer present while price moved through it
 */
function detectBrokenWalls(
  microstructure: FuturesMarketData['microstructure'],
  currentPrice: number,
  tickSize: number
): FuturesMarketData['brokenWalls'] {
  const now = Date.now();
  const fiveMinMs = 5 * 60 * 1000;

  // Clean up old broken walls (older than 5 min)
  while (brokenWallsHistory.length > 0) {
    const oldest = brokenWallsHistory[0];
    if (now - new Date(oldest.brokenAt).getTime() > fiveMinMs) {
      brokenWallsHistory.shift();
    } else {
      break;
    }
  }

  if (!microstructure?.restingLimitOrders) return brokenWallsHistory.length > 0 ? [...brokenWallsHistory] : undefined;

  // Get current significant walls (size > median * 2)
  const orders = microstructure.restingLimitOrders;
  const allSizes = orders.map(o => Math.max(o.restingBid, o.restingAsk)).filter(s => s > 0);
  const medianSize = allSizes.length > 0 ? allSizes.sort((a, b) => a - b)[Math.floor(allSizes.length / 2)] : 0;
  const significantThreshold = medianSize * 2;

  // Current walls by price
  const currentWalls = new Map<string, { price: number; side: 'bid' | 'ask'; size: number }>();
  for (const order of orders) {
    if (order.restingBid > significantThreshold) {
      const key = `bid-${order.price}`;
      currentWalls.set(key, { price: order.price, side: 'bid', size: order.restingBid });
    }
    if (order.restingAsk > significantThreshold) {
      const key = `ask-${order.price}`;
      currentWalls.set(key, { price: order.price, side: 'ask', size: order.restingAsk });
    }
  }

  // Check if any historical walls were broken
  for (const [key, wall] of wallHistory.entries()) {
    // Skip if wall is too old
    if (now - wall.lastSeen > 60000) {
    ...
    // Wall disappeared - check if price moved through it
    const wasBidWall = wall.side === 'bid';
    const priceMovedThrough = wasBidWall
      ? currentPrice < wall.price - tickSize * 2  // Price dropped below bid wall
      : currentPrice > wall.price + tickSize * 2; // Price rose above ask wall

    if (priceMovedThrough) {
      // Wall was broken!
      const direction = wasBidWall ? 'down' : 'up';
      brokenWallsHistory.push({
        price: wall.price,
        side: wall.side,
        size: wall.size,
      ...
      console.log(`🔥 Wall broken: ${wall.side.toUpperCase()} ${wall.size}@${wall.price} - price moved ${direction}`);
    }

    // Remove from history either way
    wallHistory.delete(key);
  }

  // Add current walls to history
  for (const [key, wall] of currentWalls.entries()) {
    if (!wallHistory.has(key)) {
      wallHistory.set(key, { ...wall, firstSeen: now, lastSeen: now });
    } else {
      wallHistory.get(key)!.lastSeen = now;
    }
  }

  return brokenWallsHistory.length > 0 ? [...brokenWallsHistory] : undefined;
}
...
function computeExhaustionSignals(
  ...
  // Aggregate 1m bars into 5m bars for exhaustion analysis
  const fiveMinMs = 5 * 60 * 1000;
  const fiveMinBars = aggregateBarsToTimeframe(bars, fiveMinMs);

  // Need at least 20 x 5m bars (100 minutes of data)
  if (fiveMinBars.length < 10) {
    // Not enough data - fall back to empty signals
    return [];
  }

  // Use last 20 x 5m bars (or all available if less)
  const lookbackBars = fiveMinBars.slice(-20);

  // Split into recent (last 5 bars = 25 min) vs prior (previous 15 bars = 75 min)
  const recentBars = lookbackBars.slice(-5);
  const priorBars = lookbackBars.slice(0, -5);

  // Calculate buying/selling pressure using delta (close - open direction + volume)
  const computePressure = (barList: typeof lookbackBars) => {
    let buyPressure = 0;
    let sellPressure = 0;
    for (const bar of barList) {
      ...
      if (delta > 0) {
        buyPressure += vol * (delta / Math.max(bar.high - bar.low, tickSize));
      } else if (delta < 0) {
        sellPressure += vol * (Math.abs(delta) / Math.max(bar.high - bar.low, tickSize));
      }
    }
    return { buyPressure, sellPressure };
  };

  const recent = computePressure(recentBars);
  const prior = computePressure(priorBars);

  // Exhaustion = pressure decaying (prior > recent)
  const buyDecay = clamp01((prior.buyPressure - recent.buyPressure) / Math.max(prior.buyPressure, 1));
  const sellDecay = clamp01((prior.sellPressure - recent.sellPressure) / Math.max(prior.sellPressure, 1));

  // Price progress check - are we failing to make new highs/lows?
  const recentProgress = computeProgress5m(recentBars, tickSize);
  const priorProgress = computeProgress5m(priorBars.slice(-5), tickSize);
  const failMove = priorProgress > 0 && recentProgress < priorProgress * 0.4;
  ...
  const buildSignal = (side: 'ask' | 'bid', decay: number, failed: boolean): AbsorptionSignal | null => {
    ...
    return {
      levelName: 'Exhaustion20x5m',
      price: currentPrice,
      side,
      strength: Number(strength.toFixed(2)),
      durationSec: lookbackBars.length * 5 * 60, // Duration based on actual bar count
  ...
  // Buy exhaustion (was buying, now fading) = potential reversal down (ask side signal)
  const buySignal = buildSignal('ask', buyDecay, failMove);
  // Sell exhaustion (was selling, now fading) = potential reversal up (bid side signal)
  const sellSignal = buildSignal('bid', sellDecay, failMove);
...
// Helper for 5m bar progress calculation
function computeProgress5m(
...
export async function processOpenAIDecision(
  ...
  if (openaiDecision.confidence < 75) {
    console.log(`[OpenAI] Confidence ${openaiDecision.confidence}% below 75% threshold - Not executing`);
    return { executed: false };
  }
