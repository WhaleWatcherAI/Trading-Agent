...
import { OpenAITradingDecision } from './qwenTradingAgent';
...
export class ExecutionManager {
  ...
  // WebSocket-based position tracking (replaces REST polling)
  private wsPositionData: { netQty: number; avgPrice: number; contractId?: string } | null = null;
  private wsPositionLastUpdate = 0;
  private useWebSocketPositions = false; // Enable when WS feed is connected
  ...
  /**
   * Update position from WebSocket feed (call this from GatewayUserPosition events)
   * This replaces REST polling for position state
   */
  updatePositionFromWebSocket(data: { netQty: number; avgPrice: number; contractId?: string } | null) {
    const hasActivePosition = this.activePositions.size > 0;
    const timeSinceLastEntry = Date.now() - this.lastEntryOrderTime;
    const withinGracePeriod = this.lastEntryOrderTime > 0 && timeSinceLastEntry < 120_000; // 2 minute grace

    // If WS says FLAT but we have an active position and are within grace period,
    // ignore the FLAT signal - it may be stale or initial subscription data
    if ((!data || data.netQty === 0) && hasActivePosition && withinGracePeriod) {
      console.log(`[ExecutionManager] 📡 WS Position update: FLAT (IGNORED - active position exists, ${Math.round(timeSinceLastEntry / 1000)}s since entry)`);
      return; // Don't update state - keep our position
    }

    this.wsPositionData = data;
    this.wsPositionLastUpdate = Date.now();
    this.useWebSocketPositions = true;

    if (data && data.netQty !== 0) {
      this.brokerHasPosition = true;
      console.log(`[ExecutionManager] 📡 WS Position update: ${data.netQty > 0 ? 'LONG' : 'SHORT'} ${Math.abs(data.netQty)} @ ${data.avgPrice}`);
    } else {
      this.brokerHasPosition = false;
      console.log(`[ExecutionManager] 📡 WS Position update: FLAT`);
    }
  }

  /**
   * Check if we have a position at broker (uses WS data if available, else REST)
   */
  getBrokerHasPosition(): boolean {
    return this.brokerHasPosition;
  }

  /**
   * Update order state from WebSocket feed (call this from GatewayUserOrder events)
   * This replaces REST polling for order state
   */
  updateOrderFromWebSocket(data: {
    id: string | number;
    contractId?: string;
    side: number; // 1=buy, 2=sell
    type: number; // 2=market, 3=limit, 4=stop
    status: number; // 1=working, 2=filled, 3=cancelled, etc.
    filledQty: number;
    avgFillPrice: number;
    price?: number;
    stopPrice?: number;
  }) {
    // Mark WS as active since we're receiving order events
    this.useWebSocketPositions = true;
    this.wsPositionLastUpdate = Date.now();

    const statusName = data.status === 1 ? 'WORKING' : data.status === 2 ? 'FILLED' : data.status === 3 ? 'CANCELLED' : `STATUS_${data.status}`;
    const typeName = data.type === 2 ? 'MARKET' : data.type === 3 ? 'LIMIT' : data.type === 4 ? 'STOP' : `TYPE_${data.type}`;

    console.log(`[ExecutionManager] 📋 WS Order update: ${data.side === 1 ? 'BUY' : 'SELL'} ${typeName} - ${statusName} (ID: ${data.id})`);

    // If our stop loss or take profit order was filled/cancelled, update tracking
    const active = this.getActivePosition();
    if (active) {
      if (data.id === active.stopOrderId && data.status === 2) {
        console.log(`[ExecutionManager] 🛑 Stop loss order FILLED via WS - position closed`);
        this.closePosition(this.symbol, 'stop_loss_ws');
      } else if (data.id === active.targetOrderId && data.status === 2) {
        console.log(`[ExecutionManager] 🎯 Take profit order FILLED via WS - position closed`);
        this.closePosition(this.symbol, 'take_profit_ws');
      } else if (data.id === active.stopOrderId && data.status === 3) {
        console.log(`[ExecutionManager] ⚠️ Stop loss order CANCELLED via WS`);
      } else if (data.id === active.targetOrderId && data.status === 3) {
        console.log(`[ExecutionManager] ⚠️ Take profit order CANCELLED via WS`);
      }
    }
  }

  /**
   * Enable WebSocket mode (call when user hub connects successfully)
   * This disables REST polling since WS will provide real-time updates
   */
  enableWebSocketMode() {
    this.useWebSocketPositions = true;
    this.wsPositionLastUpdate = Date.now();
    console.log('[ExecutionManager] 📡 WebSocket mode ENABLED - REST polling disabled');
  }
  ...
  private async executeWithRest(
    ...
    // CRITICAL: Skip redundant REST validation when WebSocket mode is active
    // Position and order checks were already done in executeDecision() using WS data
    if (this.useWebSocketPositions) {
      console.log(`[ExecutionManager] ✅ [ENTRY ORDER APPROVED] WebSocket mode - using cached WS position/order data (no REST calls)`);
    } else {
      // Fallback: Validate ENTRY order via REST when WebSocket not active
      console.log(`[ExecutionManager] 🔍 [ENTRY ORDER VALIDATION] Checking broker state via REST...`);
      const brokerHasPosition = await this.refreshBrokerPositionStatus();
      if (brokerHasPosition) {
        console.warn(`[ExecutionManager] ⚠️ Broker reports existing position. Proceeding but will rely on sync to avoid duplicate exits.`);
      }
      // Do not hard-block on working entry orders; log only
      try {
        ...
        const workingEntry = (openOrders?.orders || []).some((o: any) =>
          ...
          !this.isProtectiveOrder(o, undefined)
        );
        if (workingEntry) {
          console.warn(`[ExecutionManager] ⚠️ Existing working entry order detected for contract ${this.contractId}; proceeding to avoid stalled state.`);
        }
      } catch (error: any) {
        console.warn('[ExecutionManager] ⚠️ Unable to check working entry orders:', error?.message || error);
      }
      console.log(`[ExecutionManager] ✅ [ENTRY ORDER APPROVED] proceeding with entry`);
    }
    ...
    try {
      ...
      // TopstepX bracket ticks are SIGNED:
      // - Long: stopTicks negative (below entry), targetTicks positive (above entry)
      // - Short: stopTicks positive (above entry), targetTicks negative (below entry)
      // The getBracketTicks function already returns correctly signed values

      // Log the bracket calculation for debugging
      const expectedStopPrice = this.normalizePrice(entryReference + (stopTicks * this.tickSize));
      const expectedTPPrice = this.normalizePrice(entryReference + (targetTicks * this.tickSize));

      console.log(`[ExecutionManager] Bracket calculation:`);
      console.log(`  Side: ${order.side.toUpperCase()}`);
      console.log(`  Entry reference: ${entryReference} (normalized: ${this.normalizePrice(entryReference)})`);
      console.log(`  Tick size: ${this.tickSize}`);
      console.log(`  Stop ticks: ${stopTicks} (signed) -> expected price: ${expectedStopPrice}`);
      console.log(`  TP ticks: ${targetTicks} (signed) -> expected price: ${expectedTPPrice}`);
      ...
      const response = await this.restClient.placeOrder({
        ...
        stopLossBracket: {
          ticks: stopTicks, // Signed: negative for long, positive for short
          type: 4, // Stop
        },
        takeProfitBracket: {
          ticks: targetTicks, // Signed: positive for long, negative for short
          type: 1, // Limit
        },
      });
      ...
      // CRITICAL: Wait before making verification REST calls to avoid disrupting WebSocket hubs
      // TopStepX broker appears to close connections when REST activity spikes immediately after entry
      console.log('[ExecutionManager] Waiting 2s before bracket verification to stabilize WebSocket hubs...');
      ...
      if (!hasNativeBracket) {
        // CRITICAL: NO FALLBACK - Native OCO brackets ONLY
        // User requirement: "it should be using only oco.. no fall back to placing a new one only should modify by id existing"
        console.error('[ExecutionManager] ❌ CRITICAL: Native OCO bracket legs not found after verification!');
        console.error('[ExecutionManager] 🚫 NO FALLBACK TO STANDALONE ORDERS - closing naked position immediately!');

        // Close the naked position since we can't create proper OCO brackets
        try {
        ...
        // Mark order as failed - no fallback to standalone orders
        order.status = 'rejected';
        order.reason = 'Native OCO bracket creation failed - no fallback allowed';
        return null;
      }
  ...
  async executeDecision(
    ...
    // CRITICAL: Check for open positions BEFORE executing
    // This prevents duplicate entries when AI response is stale/delayed
    console.log('[ExecutionManager] 🔍 Checking for existing positions before entry...');

    // Use WebSocket data when available (no REST API call needed)
    if (this.useWebSocketPositions) {
      if (this.brokerHasPosition) {
        const wsQty = this.wsPositionData?.netQty || 0;
        console.warn(`[ExecutionManager] 🚫 WS POSITION CHECK: Position already exists (${wsQty > 0 ? 'LONG' : 'SHORT'} ${Math.abs(wsQty)} contracts). Rejecting new entry.`);
        return null;
      }
      console.log('[ExecutionManager] ✅ WS POSITION CHECK: No existing position. Safe to proceed.');
    } else {
      // Fallback to REST API when WebSocket not active
      try {
        const hasRestContext = await this.ensureRestContext();
        if (hasRestContext && this.restClient && this.tradingAccount) {
          ...
          if (existingPosition) {
            ...
            console.warn(`[ExecutionManager] 🚫 REST BROKER CHECK: Position already exists at broker (${qty > 0 ? 'LONG' : 'SHORT'} ${Math.abs(qty)} contracts). Rejecting new entry to prevent duplicate.`);
            return null;
          }
          ...
          if (pendingEntryOrders.length > 0) {
            console.warn(`[ExecutionManager] 🚫 REST BROKER CHECK: ${pendingEntryOrders.length} pending entry order(s) at broker. Rejecting new entry to prevent duplicate.`);
            return null;
          }

          console.log('[ExecutionManager] ✅ REST BROKER CHECK: No existing position or pending orders. Safe to proceed.');
        }
      } catch (error: any) {
        console.error('[ExecutionManager] ⚠️ REST broker check failed:', error.message);
  ...
  private async executeOrder(
    ...
    if (this.enableNativeBrackets) {
      // Native OCO mode - NO FALLBACK to legacy
      if (!this.restClient) {
        console.error('[ExecutionManager] ❌ Native brackets enabled but REST client not available');
        order.status = 'rejected';
        order.reason = 'REST client required for native OCO brackets';
        return null;
      }
      executionResult = await this.executeWithRest(order, currentPrice, overrides);
      if (!executionResult) {
        // NO FALLBACK - user requirement: "it should be using only oco.. no fall back"
        console.error('[ExecutionManager] 🚫 Native OCO bracket failed - NO FALLBACK to legacy standalone orders');
        order.status = 'rejected';
        order.reason = 'Native OCO execution failed - no fallback allowed';
        return null;
      }
    } else {
      // Legacy mode (native brackets disabled) - use legacy execution
   ...
   * All prices are normalized to tick size to prevent alignment errors
   */
  private calculateBracketLevels(
    ...
    // Normalize entry price first to ensure consistent calculations
    const normalizedEntry = this.normalizePrice(entryPrice);

    const defaultBuy = {
      stopLoss: this.normalizePrice(normalizedEntry - riskPoints),
      takeProfit: this.normalizePrice(normalizedEntry + rewardPoints),
    };
    const defaultSell = {
      stopLoss: this.normalizePrice(normalizedEntry + riskPoints),
      takeProfit: this.normalizePrice(normalizedEntry - rewardPoints),
    ...
    // Apply overrides but still normalize them
    return {
      stopLoss: overrides?.stopLoss ? this.normalizePrice(overrides.stopLoss) : defaults.stopLoss,
      takeProfit: overrides?.takeProfit ? this.normalizePrice(overrides.takeProfit) : defaults.takeProfit,
    };
  }
  ...
  private getBracketTicks(
    ...
    // Normalize all prices to ensure accurate tick calculations
    const normalizedEntry = this.normalizePrice(entryReference);
    const normalizedStop = this.normalizePrice(stopPrice);
    const normalizedTarget = this.normalizePrice(targetPrice);

    const rawStopTicks = Math.round((normalizedStop - normalizedEntry) / this.tickSize);
    const rawTargetTicks = Math.round((normalizedTarget - normalizedEntry) / this.tickSize);
  ...
  /**
   * Check for and clear ghost positions.
   * If agent thinks we have a position but broker is flat, clear the ghost position.
   * This handles cases where stop loss was filled but WS didn't notify us properly.
   * Returns true if a ghost position was cleared.
   */
  async checkAndClearGhostPosition(): Promise<boolean> {
    const active = this.getActivePosition();
    if (!active) {
      return false; // No position to sync
    }

    // Only check broker if we have active position
    const brokerFlat = await this.verifyPositionFlattened();
    if (brokerFlat) {
      console.log(`[ExecutionManager] 🔄 GHOST POSITION DETECTED: Broker is FLAT but agent has active ${active.side.toUpperCase()} position!`);
      console.log(`[ExecutionManager] 🛑 CLEARING GHOST: Stop loss or target was likely filled by broker`);

      // Clear the ghost position
      this.activePositions.clear();
      this.brokerHasPosition = false;

      console.log(`[ExecutionManager] ✅ Ghost position cleared - agent now FLAT`);
      return true;
    }
      ...
      if (!matching) {
        ...
        try {
          ...
          if (openOrdersResponse?.success && Array.isArray(openOrdersResponse.orders) && openOrdersResponse.orders.length > 0) {
            ...
            // CRITICAL: Mark broker as having a position since we reconstructed from bracket orders
            // This allows adjustActiveProtection to work even when positions API returns 404
            this.brokerHasPosition = true;
  ...
  private async submitBracketOrders(
    ...
    // Normalize prices to tick size to prevent "not aligned" errors
    const normalizedStopLoss = this.normalizePrice(stopLoss);
    const normalizedTakeProfit = this.normalizePrice(takeProfit);

    console.log(`[ExecutionManager] Fallback bracket prices (normalized):`);
    console.log(`  Stop Loss: ${stopLoss} -> ${normalizedStopLoss}`);
    console.log(`  Take Profit: ${takeProfit} -> ${normalizedTakeProfit}`);

    try {
      // Submit stop loss order (stop market order)
      console.log(`[ExecutionManager] Submitting STOP LOSS order @ ${normalizedStopLoss.toFixed(2)}...`);
      ...
      if (stopResult.success) {
        console.log(`[ExecutionManager] ✅ Stop loss order placed @ ${normalizedStopLoss.toFixed(2)}`);
      ...
      console.log(`[ExecutionManager] Submitting TAKE PROFIT order @ ${normalizedTakeProfit.toFixed(2)}...`);
      ...
      const takeProfitOrder = {
        ...
        limitPrice: normalizedTakeProfit,
        live: this.live,
      };
      ...
      if (profitResult.success) {
        console.log(`[ExecutionManager] ✅ Take profit order placed @ ${normalizedTakeProfit.toFixed(2)}`);
  ...
  async syncWithBrokerState(): Promise<void> {
    // When WebSocket mode is enabled, trust WebSocket for position tracking
    // Don't make REST API calls that can timeout and destabilize connections
    if (this.useWebSocketPositions) {
      // Skip ALL REST API calls in WebSocket mode
      // Ghost position detection via REST was causing 30s timeouts
      // which destabilized SignalR hub connections
      return;
    }
    ...
    console.log('[ExecutionManager] 🔄 REST sync (WebSocket not active)');
    ...
    try {
      ...
      // CRITICAL: Do NOT cancel protective orders within grace period - positions API lags behind order fills
      const timeSinceLastEntry = now - this.lastEntryOrderTime;
      const withinEntryGracePeriod = this.lastEntryOrderTime > 0 && timeSinceLastEntry < 60_000; // 60 second grace for position API lag

      if (!brokerPosition && !withinEntryGracePeriod) {
        const orphanProtective = openOrderList.filter(order =>
          order.contractId === this.contractId &&
          this.isProtectiveOrder(order, undefined)
        );
        if (orphanProtective.length > 0) {
          console.log(`[ExecutionManager] ⚠️ Flat at broker (no grace period) but found ${orphanProtective.length} protective order(s). Canceling protective orphans for safety...`);
          await this.cancelOrders(orphanProtective);
        }
      } else if (!brokerPosition && withinEntryGracePeriod) {
        console.log(`[ExecutionManager] ⏳ Broker position API shows flat, but within ${Math.round((60_000 - timeSinceLastEntry) / 1000)}s entry grace period - NOT canceling protective orders (API may be lagging)`);
      }
      ...
      if (!targetRecord) {
        ...
        // PROTECTION: Never sync a worse target from broker (protects max potential gains)
        const brokerTarget = targetRecord.limitPrice;
        const isTargetReduction = active.side === 'long'
          ? brokerTarget < active.target
          : brokerTarget > active.target;

        if (isTargetReduction) {
          console.log(`[ExecutionManager] ⚠️ Broker sync BLOCKED target reduction: ${active.target.toFixed(2)} -> ${brokerTarget.toFixed(2)} (keeping higher target)`);
        } else {
          active.target = brokerTarget;
        }
      }
    } catch (error: any) {
  ...
  /**
   * Update target
   * PROTECTION: Target can only be extended (moved further in the money), never reduced
   */
  updateTarget(decisionId: string, newTarget: number): boolean {
    const position = this.activePositions.get(decisionId);
    if (!position) return false;

    // PROTECTION: Block target reductions (protects max potential gains)
    const isTargetReduction = position.side === 'long'
      ? newTarget < position.target
      : newTarget > position.target;

    if (isTargetReduction) {
      console.log(`[ExecutionManager] ⚠️ updateTarget BLOCKED reduction: ${position.target.toFixed(2)} -> ${newTarget.toFixed(2)} (keeping higher target)`);
      return false;
    }
   ...
   */
  async adjustActiveProtection(newStop?: number | null, newTarget?: number | null, positionVersion?: number, currentMarketPrice?: number): Promise<boolean> {
    console.log(`[ExecutionManager] 🔔 adjustActiveProtection CALLED with: newStop=${newStop?.toFixed(2) ?? 'null'}, newTarget=${newTarget?.toFixed(2) ?? 'null'}, currentMarketPrice=${currentMarketPrice?.toFixed(2) ?? 'none'}`);
    ...
    // Belt-and-suspenders: if broker is flat (or qty zero), block any modifications
    ...
    if ((stopNeedsUpdate && !position.stopOrderId) || (targetNeedsUpdate && !position.targetOrderId)) {
      ...
      // NO FALLBACK - user requirement: "only should modify by id existing"
      // Cannot modify brackets without order IDs - log and return false
      console.error('[ExecutionManager] 🚫 Cannot adjust brackets without order IDs - no fallback to cancel-and-replace');
      console.error('[ExecutionManager] ⚠️ Bracket order IDs required for native OCO modification');
      return false;
    }
    ...
    if (stopNeedsUpdate) {
      ...
      // Use passed-in currentMarketPrice (live bar data) if available, otherwise fallback to position.currentPrice
      const currentPrice = currentMarketPrice ?? position.currentPrice;
      const minStopDistance = this.tickSize * 4; // Minimum 4 ticks away from market
      console.log(`[ExecutionManager] 📊 Stop validation using price: ${currentPrice.toFixed(2)} (${currentMarketPrice ? 'live market' : 'position.currentPrice'})`)
      ...
      if (position.side === 'short') {
        ...
        if (normalizedStop < minValidStop) {
          // The requested stop is too close to market - use the minimum valid stop instead
          // This still tightens the stop (moves it down toward entry for shorts)
          ...
          // Only skip if the adjusted stop would be WORSE (higher) than current stop
          if (normalizedStop > position.stopLoss) {
            console.log(`[ExecutionManager] ⚠️ SHORT stop adjustment skipped: adjusted stop ${normalizedStop.toFixed(2)} would be worse than current ${position.stopLoss.toFixed(2)}`);
            skipStopUpdate = true;
          }
        }
      } else {
        ...
        if (normalizedStop > maxValidStop) {
          // The requested stop is too close to market - use the maximum valid stop instead
          // This still tightens the stop (moves it up toward entry for longs)
          ...
          // Only skip if the adjusted stop would be WORSE (lower) than current stop
          if (normalizedStop < position.stopLoss) {
            console.log(`[ExecutionManager] ⚠️ LONG stop adjustment skipped: adjusted stop ${normalizedStop.toFixed(2)} would be worse than current ${position.stopLoss.toFixed(2)}`);
            skipStopUpdate = true;
          }
        }
      }
      ...
      if (!skipStopUpdate) {
        ...
        if (stopModified) {
          ...
          // NO FALLBACK - user requirement: "only should modify by id existing"
          console.error(`[ExecutionManager] ❌ Stop modification failed after retries - no fallback to cancel-and-replace`);
          console.error(`[ExecutionManager] ⚠️ Stop adjustment NOT applied - modification by ID required`);
        }
      }
    }

    // Modify target with retry logic
    // IMPORTANT: Target can only be EXTENDED (further in the money), never reduced
    // This protects max potential gains
    if (targetNeedsUpdate) {
      const normalizedTarget = this.normalizePrice(newTarget as number);

      // Check if this would reduce the target (make it less favorable)
      let skipTargetUpdate = false;
      if (position.side === 'long') {
        // LONG: target must be HIGHER than current target to extend gains
        if (normalizedTarget < position.target) {
          console.log(`[ExecutionManager] ⚠️ LONG target reduction BLOCKED: ${position.target.toFixed(2)} -> ${normalizedTarget.toFixed(2)}. Target can only be extended, never reduced.`);
          skipTargetUpdate = true;
        }
      } else {
        // SHORT: target must be LOWER than current target to extend gains
        if (normalizedTarget > position.target) {
          console.log(`[ExecutionManager] ⚠️ SHORT target reduction BLOCKED: ${position.target.toFixed(2)} -> ${normalizedTarget.toFixed(2)}. Target can only be extended, never reduced.`);
          skipTargetUpdate = true;
        }
      }

      if (skipTargetUpdate) {
        console.log(`[ExecutionManager] 🛡️ Protecting max potential gains - target extension only policy enforced`);
      } else {
      ...
      if (targetModified) {
        ...
        // NO FALLBACK - user requirement: "only should modify by id existing"
        console.error(`[ExecutionManager] ❌ Target modification failed after retries - no fallback to cancel-and-replace`);
        console.error(`[ExecutionManager] ⚠️ Target adjustment NOT applied - modification by ID required`);
      }
      }
    }
  ...
  private async modifyOrderWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Check for "Order not found" error (errorCode 3) - position was closed at broker
        if (result?.errorCode === 3 || result?.errorMessage?.toLowerCase().includes('order not found')) {
          console.warn(`[ExecutionManager] 🚨 ORDER NOT FOUND - Position likely closed at broker! Clearing local state.`);
          // Clear local position state to prevent ghost position loop
          this.activePosition = null;
          return false; // Don't retry - order genuinely doesn't exist
        }
  ...
  private async cancelAndReplaceTarget(position: ActivePosition, newTarget: number): Promise<boolean> {
    ...
    if (isTargetReduction) {
      console.log(`[ExecutionManager] ⚠️ cancelAndReplaceTarget BLOCKED reduction: ${position.target.toFixed(2)} -> ${newTarget.toFixed(2)} (keeping higher target)`);
      return false;
    }
  ...
  /**
   * Get trading account ID (for WebSocket subscriptions)
   */
  getAccountId(): number {
    return this.tradingAccount?.id ?? 0;
  }
  ...
  /**
   * Check broker position status (uses WebSocket data when available, falls back to REST)
   * Updates brokerHasPosition flag
   */
  async refreshBrokerPositionStatus(): Promise<boolean> {
    const now = Date.now();

    // When WebSocket mode is enabled, use cached WS data - NO REST API calls
    if (this.useWebSocketPositions) {
      ...
      // Return cached WebSocket position state - no API call needed
      return this.brokerHasPosition;
    }

    // Rate limit REST calls (every 2 seconds when WS not active)
    ...
    try {
      console.log(`[ExecutionManager] 📊 REST position check (WebSocket not active)`);
      const positions = await this.restClient.getPositions(this.tradingAccount.id);
      ...
      if (brokerPosition) {
        this.brokerHasPosition = true;
        console.log(`[ExecutionManager] 📊 Broker position check: POSITION EXISTS (via REST positions)`);
        return true;
      }

      // FALLBACK: TopstepX /api/Account/{id}/positions often returns 404 even with open positions
      // Check for open bracket orders (stop + limit) as alternative position detection
      // Order types: 1=Limit, 2=Market, 4=Stop (numeric, not string)
      try {
        const openOrders = await this.restClient.searchOpenOrders({ accountId: this.tradingAccount.id });
        const ourOrders = (openOrders?.orders || []).filter((o: any) => o.contractId === this.contractId);

        // Debug: log order types found
        if (ourOrders.length > 0) {
          console.log(`[ExecutionManager] 📋 Open orders for ${this.contractId}: ${ourOrders.length} order(s)`);
          ourOrders.forEach((o: any) => {
            console.log(`[ExecutionManager]   - Order ${o.id}: type=${o.type}, stopPrice=${o.stopPrice}, limitPrice=${o.limitPrice}`);
          });
        }

        // Type 4 = Stop order, or has stopPrice field
        const hasStopOrder = ourOrders.some((o: any) => o.type === 4 || (typeof o.stopPrice === 'number' && o.stopPrice > 0));
        // Type 1 = Limit order (take profit), without stopPrice
        const hasLimitOrder = ourOrders.some((o: any) => o.type === 1 || (typeof o.limitPrice === 'number' && o.limitPrice > 0 && !o.stopPrice));

        // If we have both stop and limit (bracket), position is open
        if (hasStopOrder && hasLimitOrder) {
          this.brokerHasPosition = true;
          console.log(`[ExecutionManager] 📊 Broker position check: POSITION EXISTS (via bracket orders - stop=${hasStopOrder}, limit=${hasLimitOrder})`);
          return true;
        }

        // Even with just one protective order, we likely have a position
        if (ourOrders.length > 0) {
          this.brokerHasPosition = true;
          console.log(`[ExecutionManager] 📊 Broker position check: POSITION EXISTS (has ${ourOrders.length} open order(s))`);
          return true;
        }
      } catch (fallbackErr: any) {
        console.warn(`[ExecutionManager] ⚠️ Bracket order fallback check failed: ${fallbackErr.message}`);
      }

      this.brokerHasPosition = false;
      console.log(`[ExecutionManager] 📊 Broker position check: FLAT`);
      return false;
    } catch (error: any) {
