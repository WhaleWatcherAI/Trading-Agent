...
/**
 * Invalidate cached session token (call this on 401 errors)
 */
export function invalidateSessionToken() {
  sessionToken = null;
  tokenExpiry = 0;
  console.log('[topstepx] Session token invalidated, will re-authenticate on next request');
}
...
export interface FetchFuturesBarsParams {
  ...
  includePartialBar?: boolean;  // true to get live/incomplete bar for real-time prices
}
...
export async function fetchTopstepXFuturesBars(
  ...
  const {
    ...
    limit = 20000,
    live = false,
    includePartialBar = false, // Include live/partial bar for real-time price updates
  } = params;
