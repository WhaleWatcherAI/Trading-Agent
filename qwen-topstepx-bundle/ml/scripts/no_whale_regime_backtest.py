#!/usr/bin/env python3
"""
No-Whale + Regime LSTM XGBoost Backtest

Enhanced version with additional Regime LSTM that uses:
- 1-min OHLCV candles
- Volume Profile (POC, VAH, VAL positions)
- CVD and CVD EMA
- ATR (14-period)
...
4. NEW: Regime LSTM - 1-min bars with all indicators (60 bars = 1 hour)
5. NEW: Timing XGBoost - Uses regime embeddings to decide IF to trade
6. Final XGBoost - Makes final TAKE/HOLD decision
"""
...
def train_lstm_models(
    ...
    # Mixed precision scaler for 50% memory reduction
    scaler = torch.cuda.amp.GradScaler() if torch.cuda.is_available() else None

    # Train long-term LSTM in batches to avoid OOM
    if len(longterm_seqs) > 100:
        ...
        X_lt_np = np.array(longterm_seqs, dtype=np.float32)
        y_lt_np = np.array(longterm_labels, dtype=np.int64)

        batch_size = 2000  # Process 2000 sequences at a time
        n_samples = len(X_lt_np)
        ...
        for epoch in range(epochs):
            ...
            for batch_start in range(0, n_samples, batch_size):
                ...
                # Load batch to GPU
                X_batch = torch.tensor(X_lt_np[batch_indices], dtype=torch.float32).to(DEVICE)
                y_batch = torch.tensor(y_lt_np[batch_indices], dtype=torch.long).to(DEVICE)

                optimizer.zero_grad()

                # Use mixed precision (FP16) if on CUDA
                if torch.cuda.is_available():
                    with torch.cuda.amp.autocast():
                        embeddings = longterm_lstm(X_batch)
        ...
        del X_lt_np, y_lt_np, classifier, optimizer
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

    # Train short-term LSTM in batches to avoid OOM
    if len(shortterm_seqs) > 100:
        ...
        # Convert to numpy arrays
        X_st_np = np.array(shortterm_seqs, dtype=np.float32)
        y_st_np = np.array(shortterm_labels, dtype=np.int64)

        batch_size = 2000  # Process 2000 sequences at a time
        n_samples = len(X_st_np)
        ...
        for epoch in range(epochs):
            ...
            for batch_start in range(0, n_samples, batch_size):
                ...
                # Load batch to GPU
                X_batch = torch.tensor(X_st_np[batch_indices], dtype=torch.float32).to(DEVICE)
                y_batch = torch.tensor(y_st_np[batch_indices], dtype=torch.long).to(DEVICE)
                ...
                if torch.cuda.is_available():
                    with torch.cuda.amp.autocast():
                        embeddings = shortterm_lstm(X_batch)
                        logits = classifier(embeddings)
                        loss = criterion(logits, y_batch)
                    scaler.scale(loss).backward()
                    scaler.step(optimizer)
        ...
        # Free memory after training
        del X_st_np, y_st_np, classifier, optimizer
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

    # Train regime LSTM in batches to avoid OOM
    if len(regime_seqs) > 100:
        ...
        # Convert to numpy arrays
        X_reg_np = np.array(regime_seqs, dtype=np.float32)
        y_reg_np = np.array(regime_labels, dtype=np.int64)

        batch_size = 2000  # Process 2000 sequences at a time
        n_samples = len(X_reg_np)
        ...
        for epoch in range(epochs):
            ...
            for batch_start in range(0, n_samples, batch_size):
                ...
                # Load batch to GPU
                X_batch = torch.tensor(X_reg_np[batch_indices], dtype=torch.float32).to(DEVICE)
                y_batch = torch.tensor(y_reg_np[batch_indices], dtype=torch.long).to(DEVICE)
                ...
                if torch.cuda.is_available():
                    with torch.cuda.amp.autocast():
                        embeddings = regime_lstm(X_batch)
                        logits = classifier(embeddings)
                        loss = criterion(logits, y_batch)
                    scaler.scale(loss).backward()
                    scaler.step(optimizer)
        ...
        # Free memory after training
        del X_reg_np, y_reg_np, classifier, optimizer
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
...
def build_final_training_data(
    ...
    """Build training data for final XGBoost with all embeddings."""
    X_all = []
    y_all = []
    ...
    for date in train_dates:
        ...
        for bar_idx in range(max(72, REGIME_SEQ_LEN), len(bars_1min) - HOLD_BARS, 3):
            ...
            # Combined features with all embeddings and timing score
...
def run_single_day(
    ...
    """Run backtest for a single day with regime filtering."""
...
def main():
    ...
    print("NO-WHALE + REGIME LSTM XGBOOST BACKTEST", file=sys.stderr)
    print("Enhanced with Regime LSTM (ATR, BB, ADX, Chop, RSI, VP, CVD)", file=sys.stderr)
    print("Architecture: Stage1 XGB -> LSTMs (3) -> Timing XGB -> Final XGB", file=sys.stderr)
    ...
    for i, test_date in enumerate(dates):
        ...
        if args.models_dir:
            ...
            # Train LSTM models with batched training (avoids OOM)
        ...
        # Clear GPU cache after each day to prevent memory buildup
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
