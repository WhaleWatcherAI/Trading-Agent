...
def run_single_day(
    ...
    for bar_idx in range(max(72, REGIME_SEQ_LEN), len(bars_1min) - HOLD_BARS):
        ...
        final_score = proba[1]
        # DEBUG LOG
        ts = bars_1min[bar_idx]["t"]
        if "2025-12-10T14" in ts or "2025-12-10T19" in ts or "2025-12-10T20" in ts or "2025-12-10T21" in ts:
            print(f"DEBUG: {ts} | Final Score: {final_score:.2f} (Pred: {pred})")
