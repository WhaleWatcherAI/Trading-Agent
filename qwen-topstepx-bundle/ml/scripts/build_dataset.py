...
# Prefer local trading-db inside bundle, but also allow sibling repo trading-db
def _resolve_db_file(name: str) -> Path:
    candidates = [
        ROOT / "trading-db" / name,            # bundled path
        ROOT.parent / "trading-db" / name,     # sibling to bundle
    ]
    for cand in candidates:
        if cand.exists():
            return cand
    # fall back to first candidate to keep paths consistent for errors
    return candidates[0]

DECISIONS_PATH = _resolve_db_file("decisions.jsonl")
OUTCOMES_PATH = _resolve_db_file("outcomes.jsonl")
