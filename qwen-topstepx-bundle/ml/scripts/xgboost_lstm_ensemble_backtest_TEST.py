#!/usr/bin/env python3
"""
Multi-Stage XGBoost + LSTM Ensemble Backtest - TEST VERSION

This is a test copy for experimentation without affecting production models.
...
def calculate_sma(values: List[float], period: int) -> List[float]:
    ...
    for i in range(len(values)):
        ...
        sma[i] = sum(window) / len(window) if window else 0.0
    return sma
...
class HourLSTM(nn.Module):
    """
    1-hour LSTM using simple price-action features (OHLCV + SMA distances).
    Output: 32-dim embedding
    """
    def __init__(self, input_dim: int = 8, hidden_dim: int = 96, num_layers: int = 2, embedding_dim: int = 32):
        super().__init__()
        self.lstm = nn.LSTM(input_dim, hidden_dim, num_layers, batch_first=True, dropout=0.2)
        self.fc = nn.Linear(hidden_dim, embedding_dim)
        self.layer_norm = nn.LayerNorm(embedding_dim)

        # Small head for auxiliary training
...
def extract_hour_sequence(
    bars_1h: List[Dict],
    sma50: List[float],
    sma100: List[float],
    sma200: List[float],
    bar_idx_1h: int,
    seq_len: int = 60,  # 60 hours lookback
) -> Optional[np.ndarray]:
    """
    Extract 1-hour sequence for simplified branch (no VP/CVD).
    Features per 1h bar:
    - OHLCV normalized to close
    - SMA distances (50/100/200) with no leakage
    """
    if bar_idx_1h < seq_len or bar_idx_1h >= len(bars_1h):
        return None

    seq = []
    for j in range(bar_idx_1h - seq_len, bar_idx_1h):
        bar = bars_1h[j]
        close = bar['c'] or 1.0
    ...
    return np.array(seq, dtype=np.float32) if len(seq) == seq_len else None
...
def build_training_data_with_lstm(
    ...
    for date in train_dates:
        ...
        sma50_1h = calculate_sma(closes_1h, 50)
        sma100_1h = calculate_sma(closes_1h, 100)
        sma200_1h = calculate_sma(closes_1h, 200)
        ...
        for i in range(max(72, 50), len(bars_1min) - HOLD_BARS, 5):
            ...
            hour_idx = i // 60
            hour_seq = extract_hour_sequence(bars_1h, sma50_1h, sma100_1h, sma200_1h, hour_idx, seq_len=60)
            if hour_seq is None:
                continue
            ...
            with torch.no_grad():
                ...
                hour_tensor = torch.tensor(hour_seq, dtype=torch.float32).unsqueeze(0).to(DEVICE)
                ...
                hour_emb, _ = hour_lstm(hour_tensor)
                hour_emb = hour_emb.cpu().numpy().flatten()

            # Combine features
            xgb_feat_array = np.array(list(xgb_features.values()))
            combined = np.concatenate([xgb_feat_array, longterm_emb, shortterm_emb, hour_emb])
...
def train_lstm_models(
    ...
    for date in train_dates:
        ...
        # Hourly sequences (1-hour bars, OHLCV+SMA)
        if len(bars_1h) > 400:  # need enough history for SMA200 + seq_len
            closes_1h = [b['c'] for b in bars_1h]
            sma50 = calculate_sma(closes_1h, 50)
            sma100 = calculate_sma(closes_1h, 100)
            sma200 = calculate_sma(closes_1h, 200)

            for i in range(200 + 60, len(bars_1h) - 1, 3):
                seq = extract_hour_sequence(bars_1h, sma50, sma100, sma200, i, seq_len=60)
                if seq is not None:
                    future_price = bars_1h[i]['c']
                    current_price = bars_1h[i - 1]['c']
                    label = 1 if future_price > current_price else 0
                    hour_seqs.append(seq)
    ...
    hour_lstm = HourLSTM(input_dim=8, hidden_dim=96, num_layers=NUM_LAYERS, embedding_dim=EMBEDDING_DIM).to(DEVICE)
    ...
    # Train hour LSTM (auxiliary next-hour classification)
    if len(hour_seqs) > 100:
        X_hr = torch.tensor(np.array(hour_seqs), dtype=torch.float32).to(DEVICE)
        y_hr = torch.tensor(np.array(hour_labels), dtype=torch.long).to(DEVICE)

        optimizer = torch.optim.Adam(hour_lstm.parameters(), lr=0.001)
        criterion = nn.CrossEntropyLoss()
        hour_lstm.train()
        for epoch in range(epochs):
            optimizer.zero_grad()
            embeddings, logits = hour_lstm(X_hr)
            loss = criterion(logits, y_hr)
            loss.backward()
            optimizer.step()

    return longterm_lstm, shortterm_lstm, hour_lstm
...
def run_single_day(
    ...
    shortterm_lstm: nn.Module,
    hour_lstm: nn.Module,
    iv_rank: Optional[Dict[str, float]],
    oi_changes: Optional[Dict[str, float]],
    options_premium_threshold: float,
    ...
    for _, row in filtered_options.iterrows():
        ...
        if bar_idx == 0:
            continue
        vp = calculate_volume_profile(bars_1min[:bar_idx], lookback=30)
...
def main():
    ...
    for i, test_date in enumerate(dates):
        ...
        # Train LSTM models
        longterm_lstm, shortterm_lstm, hour_lstm = train_lstm_models(
        ...
        X_train, y_train = build_training_data_with_lstm(
            ...
            longterm_lstm, shortterm_lstm, hour_lstm
        )
    ...
    output_path = "data/xgboost_lstm_ensemble_results_TEST.json"
    with open(output_path, 'w') as f:
        json.dump(result, f, indent=2)
    print(f"\n[Save] TEST Results saved to {output_path}", file=sys.stderr)
