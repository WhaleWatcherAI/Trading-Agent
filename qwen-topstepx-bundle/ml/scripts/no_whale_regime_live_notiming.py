#!/usr/bin/env python3
"""
No-Whale Regime Live Trading with TopstepX - NO TIMING FILTER VERSION

For MICRO contracts (MNQ, MES, MGC) - generates MORE signals by removing
the timing XGBoost filter. Only uses Stage1 direction + Final confidence.
...
4. Train with tiered validation (multiple holdout days) - NO timing XGB
5. Cache live bars in memory, continuously appending
"""

# Flag to identify this as the no-timing version
NO_TIMING_VERSION = True
...
from no_whale_regime_backtest import (
    ...
    aggregate_bars, find_swing_highs_lows, detect_trade_trigger, simulate_trade,
    # Training functions - NOT importing train_timing_xgboost or build_final_training_data (we override them)
    train_lstm_models, train_stage1_xgboost,
    # Backtest/simulation functions - NOT importing run_single_day (we override it)
    # Constants
...
def get_symbol_from_contract(contract_id):
    ...
    # Micros (notiming typically uses these)
...
# Paths - contract-specific data and models (separate from timing version)
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(SCRIPT_DIR, '..', 'data')
MODELS_DIR = os.path.join(SCRIPT_DIR, '..', 'models', f'{CONTRACT_SYMBOL}_notiming')
LOGS_DIR = os.path.join(SCRIPT_DIR, '..', 'logs')
HISTORICAL_BARS_FILE = os.path.join(DATA_DIR, f'bars_1s_{CONTRACT_SYMBOL}.json')  # Share data with full version

# Create models directory if needed
os.makedirs(MODELS_DIR, exist_ok=True)

# Model files - NO timing_xgb in this version
MODEL_FILES = {
    'stage1_xgb': os.path.join(MODELS_DIR, 'stage1_xgb.json'),
    # 'timing_xgb' REMOVED - this version skips timing filter
...
def setup_logging():
    ...
    log_file = os.path.join(LOGS_DIR, f'live_notiming_{CONTRACT_SYMBOL}_{datetime.utcnow().strftime("%Y%m%d")}.log')
...
# =============================================================================
# NO-TIMING VERSION: Custom training and validation functions
# =============================================================================

def build_final_training_data_notiming(
    ...
    """Build training data for final XGBoost WITHOUT timing features.
    Uses constant timing_confidence=0.5 since we skip timing filter."""
    ...
    for date in train_dates:
        ...
        for bar_idx in range(max(72, REGIME_SEQ_LEN), len(bars_1min) - HOLD_BARS, 3):
            ...
            # NO TIMING - use constant confidence of 0.5
            ...
            # Combined features with all embeddings (timing_confidence is constant)
            ...
            if result['pnl_points'] > min_pnl_threshold:
                label = 1
            elif result['pnl_points'] < -min_pnl_threshold:
                label = 0
            else:
                continue
...
def run_single_day_notiming(
    ...
    """Run backtest for a single day WITHOUT timing filter."""
    ...
    for bar_idx in range(max(72, REGIME_SEQ_LEN), len(bars_1min) - HOLD_BARS):
        ...
        # Skip if Stage 1 confidence too low
        ...
        # NO TIMING CHECK - use constant 0.5
        timing_confidence = 0.5

        # Final decision
        xgb_arr = np.array(list(xgb_features.values()))
        final_features = np.concatenate([
            xgb_arr, longterm_emb, shortterm_emb, regime_emb,
            np.array([stage1_confidence, timing_confidence])
        ])

        final_proba = final_xgb.predict_proba(final_features.reshape(1, -1))[0]
        final_confidence = final_proba[1]

        # Only check final confidence (no timing check)
        if final_confidence < 0.55:
            continue

        # Calculate SL/TP
        # Use 5m swings (20-bar lookback) for stops/targets
        swing_highs, swing_lows = find_swing_highs_lows(bars_5min[:bar_idx//5+1], lookback=20)

        if stage1_direction == "long":
            valid_lows = [sl for sl in swing_lows if sl < current_price] if swing_lows else []
            stop_loss = min(valid_lows) - 1 if valid_lows else current_price - STOP_LOSS_POINTS
            risk = current_price - stop_loss
            target = current_price + (risk * 3)
        else:
            valid_highs = [sh for sh in swing_highs if sh > current_price] if swing_highs else []
            stop_loss = max(valid_highs) + 1 if valid_highs else current_price + STOP_LOSS_POINTS
            risk = stop_loss - current_price
            target = current_price - (risk * 3)
        ...
        # Calculate exit bar index from entry + bars_held
        exit_bar_idx = bar_idx + result['bars_held']
        ...
        current_position_exit_bar = exit_bar_idx

        # Track drawdown
        intraday_pnl += result['pnl_points']
        if intraday_pnl > intraday_peak_pnl:
            intraday_peak_pnl = intraday_pnl
        dd = intraday_peak_pnl - intraday_pnl
        if dd > max_intraday_dd:
            max_intraday_dd = dd
        if intraday_pnl < 0 and abs(intraday_pnl) > max_intraday_dd_from_peak:
            max_intraday_dd_from_peak = abs(intraday_pnl)

        cumulative_equity += result['pnl_points']
        if cumulative_equity > peak_equity:
            peak_equity = cumulative_equity

    analysis = {
        "max_intraday_dd_points": max_intraday_dd,
        "max_intraday_dd_from_peak_points": max_intraday_dd_from_peak,
    }
...
def train_with_tiered_validation(bars_1s: List[Dict]) -> Tuple[Dict, Dict]:
    ...
    # NO TIMING XGBoost in this version - skip it entirely
    if logger:
        logger.info("  [NO-TIMING VERSION] Skipping timing XGBoost training")

    # Build final training data WITHOUT timing (using constant timing_confidence)
    final_X, final_y = build_final_training_data_notiming(
        days, train_dates, stage1_xgb,
    ...
    for val_date in val_dates:
        ...
        # Run backtest on this day (NO-TIMING version - no timing_xgb)
        # Returns: (trades_list, analysis_dict, cumulative_equity, peak_equity)
        trades_list, analysis, _, _ = run_single_day_notiming(
            val_day_bars,
            val_date,
            stage1_xgb, final_xgb,
            longterm_lstm, shortterm_lstm, regime_lstm,
        )
...
def save_models(models: Dict, validation_results: Dict = None):
    """Save all models to disk (NO-TIMING version)."""
    os.makedirs(MODELS_DIR, exist_ok=True)

    # Save XGBoost models - NO timing_xgb
    models['stage1_xgb'].save_model(MODEL_FILES['stage1_xgb'])
    # timing_xgb not saved in no-timing version
...
def load_models() -> Optional[Dict]:
    ...
    try:
        # Load XGBoost models - NO timing_xgb
        stage1_xgb = xgb.XGBClassifier()
        stage1_xgb.load_model(MODEL_FILES['stage1_xgb'])

        # timing_xgb not loaded in no-timing version
...
def generate_signal(models: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        # Backtest DOES filter on stage1 confidence >= 0.6 (see run_single_day_notiming line 722)
        ...
        # NO TIMING CHECK in this version - always pass timing, use constant 0.5
        timing_confidence = 0.5
        timing_pass = True  # Always pass since no timing filter
        ...
        # Backtest uses probability threshold >= 0.55 (see run_single_day_notiming line 777)
        ...
        # Log snapshot (no timing in this version)
        s1_mark = "OK" if stage1_pass else "LOW"
        fn_mark = "OK" if final_pass else "LOW"

        all_pass = stage1_pass and final_pass  # NO timing check
        status = "SIGNAL" if all_pass else "NO TRADE"

        if logger:
            logger.info(f"SNAPSHOT | Price: {current_price:.2f} | Stage1: {stage1_confidence:.0%} ({s1_mark}) | Final: {final_confidence:.0%} ({fn_mark}) | Dir: {direction_str} | {status}")

        # Only return signal if stage1 AND final pass (NO timing check)
        if not all_pass:
            return None
        ...
        # Minimum SL distance based on contract (prevents tiny stops)
        if 'MNQ' in TOPSTEPX_CONTRACT_ID or 'ENQ' in TOPSTEPX_CONTRACT_ID:
            min_sl_points = 30  # 120 ticks for NQ
        elif 'MES' in TOPSTEPX_CONTRACT_ID or 'EP' in TOPSTEPX_CONTRACT_ID:
            min_sl_points = 8  # 32 ticks for ES
        elif 'MGC' in TOPSTEPX_CONTRACT_ID or 'GC' in TOPSTEPX_CONTRACT_ID:
            min_sl_points = 3  # 30 ticks for GC
        else:
            min_sl_points = 10
        ...
        if direction == 'buy':
            ...
            valid_lows = [sl for sl in swing_lows if sl < current_price] if swing_lows else []
            stop_loss = min(valid_lows) - 1 if valid_lows else current_price - min_sl_points
            # Enforce minimum SL distance
            if current_price - stop_loss < min_sl_points:
                stop_loss = current_price - min_sl_points
            risk = current_price - stop_loss
            take_profit = current_price + (risk * 3)  # 3:1 R:R
        else:
            # For shorts: SL above recent swing high, TP at 3:1 R:R
            valid_highs = [sh for sh in swing_highs if sh > current_price] if swing_highs else []
            stop_loss = max(valid_highs) + 1 if valid_highs else current_price + min_sl_points
            # Enforce minimum SL distance
            if stop_loss - current_price < min_sl_points:
                stop_loss = current_price + min_sl_points
            risk = stop_loss - current_price
            take_profit = current_price - (risk * 3)  # 3:1 R:R
...
def place_order(direction: str, price: float, stop_loss: float, take_profit: float) -> bool:
    ...
    try:
        ...
        # For micros (MNQ/MES/MGC), send a single order sized to 5 contracts so Topstep shows one trade
        order_size = 5 if any(x in TOPSTEPX_CONTRACT_ID for x in ['MNQ', 'MES', 'MGC']) else 1
...
def fetch_bracket_order_ids():
    ...
    try:
        ...
        stop_ids = set(active_position_state.get('stop_order_ids', []))
        tp_ids = set(active_position_state.get('tp_order_ids', []))
        ...
        for order in orders:
            if order.get('contractId') == TOPSTEPX_CONTRACT_ID:
                ...
                if order_type == 4:
                    if order_id not in stop_ids:
                        stop_ids.add(order_id)
                        if logger:
                            logger.info(f"  Stop Loss Order ID: {order_id}")
                elif order_type == 1:
                    if order_id not in tp_ids:
                        tp_ids.add(order_id)
                        if logger:
                            logger.info(f"  Take Profit Order ID: {order_id}")

        active_position_state['stop_order_ids'] = list(stop_ids)
        active_position_state['tp_order_ids'] = list(tp_ids)
    ...
    try:
                ...
                for pos in positions:
                    if pos.get('contractId') == TOPSTEPX_CONTRACT_ID:
                        ...
                        if abs(size) > 0:
                            ...
                            # Persist position state so timeout/cleanup works after restarts
                            active_position_state['has_position'] = True
                            active_position_state['side'] = 'long' if size > 0 else 'short'
                            active_position_state['entry_price'] = avg_price
                            active_position_state['contracts'] = abs(size)
                            active_position_state.setdefault('bars_held', 0)
                            active_position_state.setdefault('entry_time', datetime.utcnow())
                            if logger:
        ...
        try:
            ...
            if orders_response.ok:
                ...
                sl_orders = [o for o in orders if o.get('contractId') == TOPSTEPX_CONTRACT_ID and o.get('type') == 4]
                if sl_orders:
                    # If we restarted mid-position, mark it so timeout can trigger
                    if not active_position_state.get('has_position'):
                        active_position_state['has_position'] = True
                        active_position_state.setdefault('bars_held', 0)
                        active_position_state.setdefault('entry_time', datetime.utcnow())
                    if logger:
...
def cancel_bracket_orders():
    ...
    orders_to_cancel = []
    for sid in active_position_state.get('stop_order_ids', []):
        orders_to_cancel.append(('Stop Loss', sid))
    for tid in active_position_state.get('tp_order_ids', []):
        orders_to_cancel.append(('Take Profit', tid))
