#!/usr/bin/env tsx
/**
 * TopstepX Mean Reversion Backtest on 1-Minute Bars
 *
 ...
 * - Target Time: ~1-5 minutes typical hold
 */
...
const DEFAULT_MR_SYMBOL = process.env.TOPSTEPX_MR_SYMBOL || '6EZ5';
...
const CONFIG: BacktestConfig = {
  ...
  rsiOversold: Number(process.env.TOPSTEPX_MR_RSI_OVERSOLD || '33'),
  rsiOverbought: Number(process.env.TOPSTEPX_MR_RSI_OVERBOUGHT || '67'),
  stopLossPercent: Number(process.env.TOPSTEPX_MR_STOP_LOSS_PERCENT || '0.0001'),
  takeProfitPercent: Number(process.env.TOPSTEPX_MR_TAKE_PROFIT_PERCENT || '0.007'),
  ...
  trailingStopPercent: Number(process.env.TOPSTEPX_MR_TRAILING_STOP_PERCENT || '0.007'),
...
async function runBacktest() {
  console.log('\n' + '='.repeat(80));
  console.log('TOPSTEPX MEAN REVERSION BACKTEST (1-MINUTE BARS)');
  ...
  for (let i = 0; i < bars.length; i++) {
    ...
    if (position) {
      ...
      // Best strategy for 1-minute bars: take profit quickly at middle band
      if (false && !position.scaled && position.target !== null) {
        ...
        if (hitTarget) {
          ...
          console.log(
            `[${bar.timestamp}] 0.05% TP hit ${CONFIG.symbol} ${position.side.toUpperCase()}: ` +
