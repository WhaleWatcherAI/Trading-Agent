...
async function runNov6Backtest5Min() {
  console.log('🚀 Running Mean Reversion Backtest for November 6, 2025 (5-MINUTE BARS)\n');

  // Run backtest with 5-minute bars for signals, 1-minute for exits
  const result = await backtestMeanReversion('SPY', '2025-11-06', 'intraday', 5, 1);

  console.log('\n📊 BACKTEST RESULTS FOR SPY - November 6, 2025 (5-MIN BARS)');
  ...
  result.trades.forEach((trade, idx) => {
    ...
    if (trade.futures) {
      console.log('');
      console.log('FUTURES P&L:');
      console.log(`  ES (2 contracts):  ${profitSign}$${trade.futures.esProfit.toFixed(2)}`);
      console.log(`  MES (2 contracts): ${profitSign}$${trade.futures.mesProfit.toFixed(2)}`);
      console.log(`  SPX Points:        ${trade.futures.indexPoints.toFixed(2)}`);
      console.log(`  Commission:        $${trade.futures.commission.toFixed(2)}`);
    }
  ...
  // Calculate total futures P&L
  const totalESProfit = result.trades.reduce((sum, t) => sum + (t.futures?.esProfit ?? 0), 0);
  const totalMESProfit = result.trades.reduce((sum, t) => sum + (t.futures?.mesProfit ?? 0), 0);

  if (totalESProfit !== 0 || totalMESProfit !== 0) {
    console.log(`\n📊 FUTURES TRADING:`);
    console.log(`  ES (2 contracts):  $${totalESProfit.toFixed(2)}`);
    console.log(`  MES (2 contracts): $${totalMESProfit.toFixed(2)}`);
  }
...
runNov6Backtest5Min().catch(error => {
