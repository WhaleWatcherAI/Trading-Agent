...
async function runBacktest() {
  ...
  if (!bars.length) {
    throw new Error('No 5-minute bars returned for configured window.');
  }

  bars.reverse();
  console.log(`Loaded ${bars.length} five-minute bars`);
  ...
  for (let i = 0; i < bars.length; i++) {
    ...
    const ttmBars = bars.slice(Math.max(0, i - 10), i + 1);
    const ttmSqueeze = calculateTtmSqueeze(ttmBars, { lookback: 10, bbStdDev: 2, atrMultiplier: 1.5 });
