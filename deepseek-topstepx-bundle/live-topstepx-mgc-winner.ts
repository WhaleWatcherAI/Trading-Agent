#!/usr/bin/env tsx
/**
 * TopstepX Live MGC Winner Strategy - ENHANCED VERSION
 *
 * Features:
...
const CONFIG: StrategyConfig = {
  symbol: process.env.TOPSTEPX_NQ_LIVE_SYMBOL || 'GCZ5',
...
const DASHBOARD_PORT = Number(process.env.TOPSTEPX_NQ_DASHBOARD_PORT || '3335');
...
const TRADE_LOG_FILE = process.env.TOPSTEPX_NQ_TRADE_LOG || './logs/topstepx-mgc-winner.jsonl';
const TRADE_LOG_DIR = path.dirname(TRADE_LOG_FILE);
const STATE_FILE = './logs/.mgc-state.json';
...
let lastBarProcessedTime: Date | null = null;
...
async function enterPosition(
  ...
  const targetTicks = side === 'long' ? 23 : -23; // Positive for LONG, negative for SHORT
...
async function startMarketStream(contractId: string) {
  ...
  const handleQuote = (_contractId: string, quote: any) => {
    quoteCount++;
    if (quoteCount === 1 || quoteCount % 500 === 0) {
      log(`[HEARTBEAT] Received ${quoteCount} quotes, last bar processed: ${lastBarProcessedTime ? new Date().getTime() - lastBarProcessedTime.getTime() : 'never'}ms ago`);
    }
    if (quote) {
...
async function main() {
  console.log('\n' + '='.repeat(80));
  console.log('TOPSTEPX LIVE MGC WINNER STRATEGY - ENHANCED VERSION');
  ...
  // Start heartbeat monitor to detect if bars stop processing
  heartbeatInterval = setInterval(() => {
    const now = new Date();
    const timeSinceLastBar = lastBarProcessedTime
      ? now.getTime() - lastBarProcessedTime.getTime()
      : null;

    // Alert if no bar processed in 3 minutes (markets could be slow/halted)
    if (timeSinceLastBar && timeSinceLastBar > 180000) {
      log(`⚠️ [HEARTBEAT ALERT] No bar processed in ${Math.floor(timeSinceLastBar / 1000)}s - quotes: ${quoteCount}`);
      log(`⚠️ [HEARTBEAT] Attempting to resubscribe to market data...`);
      if (marketHub && marketHub.state === 'Connected') {
        const contractId = metadata?.contractId;
        if (contractId) {
          marketHub.invoke('SubscribeContractQuotes', contractId)
            .then(() => log(`[HEARTBEAT] ✅ Resubscribed to quotes`))
            .catch(err => log(`[HEARTBEAT] ❌ Resubscribe failed: ${err?.message}`));
          marketHub.invoke('SubscribeContractTrades', contractId)
            .then(() => log(`[HEARTBEAT] ✅ Resubscribed to trades`))
            .catch(err => log(`[HEARTBEAT] ❌ Resubscribe failed: ${err?.message}`));
        }
      }
    }
  ...
  log(`[HEARTBEAT] Monitor started - checking bar processing every 60s`);
