#!/usr/bin/env python3
"""
PPO implementation of Volume Profile Strategy
...
class VolumeProfileConfig:
    ...
    lookback = 200  # bars for volume profile calculation
    n_rows = 100  # number of price levels
    ...
    # PPO hyperparameters
    state_dim = 30  # Volume profile features + indicators
...
class VolumeProfileEnvironment:
    """Environment implementing Volume Profile strategy logic."""
    ...
    def _calculate_volume_profile(self, idx):
        """Calculate Volume Profile (POC, VAH, VAL) for current window."""
        ...
        # Expand from POC to capture value area
        ...
        # Simplified rolling approximation for real-time
        ...
        return approx_poc, approx_vah, approx_val, poc
    ...
    def _get_state(self):
        """Get current state including Volume Profile features."""
        ...
        dist_to_vah = (price - vah) / value_range
        dist_to_val = (price - val) / value_range
        dist_to_poc = (price - poc) / value_range
        ...
        features = [
            ...
            dist_to_vah,
            dist_to_val,
            value_range / price,  # Normalized value range
            ...
            # Volume features
            row['volume_ratio'],
            np.log1p(row['volume'] / 1e6),  # Log scaled volume

            # Technical indicators
            row['rsi'] / 100,
            row['atr'] / price if price > 0 else 0,
    ...
    def step(self, action):
        ...
        # Map action to position change based on strategy logic
...
def train_volume_profile():
    """Train PPO on Volume Profile strategy."""

    print("\n" + "="*70)
    print("🚀 TRAINING PPO WITH VOLUME PROFILE STRATEGY")
    print("="*70)
    ...
    print(f"\n⚙️ Configuration:")
    print(f"  Lookback: {config.lookback} bars")
    print(f"  Value Area: {config.value_area_pct:.0%}")
    print(f"  State dim: {config.state_dim}")
    print(f"  Actions: Strong Sell, Sell, Hold, Buy, Strong Buy")
                ...
                if len(episode_returns) % 10 == 0:
                    ...
                    if len(episode_returns) % 50 == 0:
                        ...
                        if val_return > best_val_return:
                            ...
                            torch.save(agent.network.state_dict(), MODELS_DIR / 'ppo_volume_profile.pth')
    ...
    # Load best model
    agent.network.load_state_dict(torch.load(MODELS_DIR / 'ppo_volume_profile.pth'))
    ...
    # Save metrics
    metrics = {
        'model': 'PPO_Volume_Profile',
        'strategy': 'Mean Reversion & Continuation',
    ...
    with open(MODELS_DIR / 'ppo_volume_profile_metrics.json', 'w') as f:
        json.dump(metrics, f, indent=2)

    print("\n" + "="*70)
    if test_return > 0.02:  # 2% return threshold
        print(f"✅ SUCCESS! Volume Profile strategy achieved {test_return:.2%} return!")
    else:
        print(f"📊 Volume Profile strategy completed with {test_return:.2%} return")
    print("="*70)
...
if __name__ == "__main__":
    ...
    agent, metrics = train_volume_profile()