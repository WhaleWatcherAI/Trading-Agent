#!/usr/bin/env tsx
/**
 * SMA Crossover Backtest - 5-MIN BARS
 *
 * - SMA calculated on 5-min bars
 * - Entry/Exit on 5-min bar closes
 * - No lookahead bias - processes bars sequentially
 * - RSI calculated on 15-min bars
 */
...
export async function runRealtime5MinSMABacktest(
  contractId: string,
  startDate: string,
  endDate: string,
  ...
  console.log('\n' + '='.repeat(80));
  console.log('REAL-TIME SMA BACKTEST - 5-MIN BARS');
  ...
  // Map 5-min bars to their corresponding RSI value
  function getRSIForTimestamp(timestamp: string): number | null {
    const time = new Date(timestamp).getTime();
    // Find the 15-min bar this 5-min bar belongs to
  ...
  // Backtest loop - process 5-min bars sequentially
  const closes5min: number[] = [];
  ...
  for (let i = 0; i < bars5min.length; i++) {
    ...
    closes5min.push(bar.close);

    // Calculate SMA on 5-min data
    const currentSMA = calculateSMA(closes5min, smaPeriod);
...
async function main() {
  ...
  console.log('\nTESTING: MICRO NASDAQ (MNQ) - SMA(10) on 5-min bars\n');
  await runRealtime5MinSMABacktest('CON.F.US.MNQ.Z25', START, END, 10, 14, 0.001, 0.011, 0.37);

  console.log('\n\nTESTING: MICRO E-MINI S&P (MES) - SMA(10) on 5-min bars\n');
  await runRealtime5MinSMABacktest('CON.F.US.MES.Z25', START, END, 10, 14, 0.001, 0.011, 0.37);
}
