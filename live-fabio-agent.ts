#!/usr/bin/env tsx
/**
 * Fabio Agent - AI-Powered Trading with Real Market Data
 *
 * Features:
...
function nextTradeId() {
  tradeSequence += 1;
  return `FABIO-${Date.now()}-${tradeSequence}`;
}

// Emit LLM decision prompt to dashboard
function emitLLMDecision(prompt: string, analysis: string, decision: string) {
  const message = `🤖 LLM Analysis:\n${prompt}\n\nMarket Context: ${analysis}\n\nDecision: ${decision}`;
  log(message);

  // Also emit as a special LLM event for the dashboard
  io.emit('llm_prompt', {
    timestamp: nowIso(),
    prompt,
    analysis,
...
async function processBar(bar: TopstepXFuturesBar) {
  ...
  if (!pendingSetup && longSetupDetected) {
    // Emit LLM decision for setup detection
    emitLLMDecision(
      "Should we prepare for a LONG position entry?",
      `Price: ${bar.close.toFixed(2)} | RSI: ${currentRSI.toFixed(1)} (oversold < ${CONFIG.rsiOversold}) | BB Lower: ${bb.lower.toFixed(2)} | TTM Squeeze: ${ttmSqueeze.squeezeOn ? 'FIRING' : 'OFF'}`,
      `✅ LONG setup detected - Stage 1 conditions met. Monitoring for TTM Squeeze confirmation...`
    );
    ...
    // Emit LLM decision for setup detection
    emitLLMDecision(
      "Should we prepare for a SHORT position entry?",
      `Price: ${bar.close.toFixed(2)} | RSI: ${currentRSI.toFixed(1)} (overbought > ${CONFIG.rsiOverbought}) | BB Upper: ${bb.upper.toFixed(2)} | TTM Squeeze: ${ttmSqueeze.squeezeOn ? 'FIRING' : 'OFF'}`,
      `✅ SHORT setup detected - Stage 1 conditions met. Monitoring for TTM Squeeze confirmation...`
    );
  ...
  if (pendingSetup && ttmSqueeze.squeezeOn) {
    ...
    // Emit LLM decision for entry trigger
    emitLLMDecision(
      `Should we EXECUTE the ${setup.side.toUpperCase()} entry now?`,
      `TTM Squeeze: FIRING | Momentum: ${ttmSqueeze.momentum.toFixed(2)} | Current Price: ${bar.close.toFixed(2)} | Setup Price: ${setup.setupPrice.toFixed(2)} | RSI: ${currentRSI.toFixed(1)}`,
      `🚀 EXECUTING ${setup.side.toUpperCase()} ENTRY - TTM Squeeze confirmation received! Placing market order...`
    );
...
async function main() {
  console.log('\n' + '='.repeat(80));
  console.log('🧠 FABIO AGENT - AI-POWERED TRADING WITH REAL MARKET DATA');
