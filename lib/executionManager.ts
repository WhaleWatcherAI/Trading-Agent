...
export class ExecutionManager {
  ...
  private async executeWithRest(
    ...
    try {
      ...
      if (!hasNativeBracket) {
        console.error('[ExecutionManager] ❌ CRITICAL: Native bracket legs not found after verification. Refusing to place standalone protective orders.');
        console.error('[ExecutionManager] 🚫 EMERGENCY: Closing potentially naked position at broker immediately!');
        ...
        order.status = 'rejected';
        order.reason = 'Native bracket creation failed - position closed to avoid naked exposure';
        return null;
      }
  ...
  private async executeOrder(
    ...
    if (this.enableNativeBrackets) {
      ...
      if (!executionResult) {
        console.error('[ExecutionManager] 🚫 Native bracket execution failed - refusing to place standalone protective orders.');
        order.status = 'rejected';
        order.reason = 'Native bracket execution failed';
        return null;
      }
    } else {
   ...
   * Enhanced with detailed logging and retry logic (no standalone fallback orders)
   */
  async adjustActiveProtection(newStop?: number | null, newTarget?: number | null): Promise<boolean> {
    ...
    if ((stopNeedsUpdate && !position.stopOrderId) || (targetNeedsUpdate && !position.targetOrderId)) {
      ...
      console.error('[ExecutionManager] 🚫 Missing bracket IDs prevents modification. Manual intervention required; skipping to avoid standalone protective orders.');
      return false;
    }
    ...
    if (stopNeedsUpdate) {
      ...
      if (!skipStopUpdate) {
        ...
        if (stopModified) {
          ...
          console.error(`[ExecutionManager] ❌ Stop modification failed after retries - no further action taken to avoid standalone stop orders.`);
        }
      }
    }
    ...
    if (targetNeedsUpdate) {
      ...
      if (targetModified) {
        ...
        console.error(`[ExecutionManager] ❌ Target modification failed after retries - no further action taken to avoid standalone target orders.`);
      }
    }
