...
const globalRef = globalThis as typeof globalThis & {
  __topstepxSmaState?: SmaProcessState;
};

const SCRIPT_PATH = path.join(process.cwd(), 'live-topstepx-sma-second.ts');
const MAX_LOG_LINES = 200;

if (!globalRef.__topstepxSmaState) {
  globalRef.__topstepxSmaState = {
    child: null,
    logs: [],
...
export function getTopstepxSmaSecondStatus(): StrategyProcessStatus {
  return {
    running: !!state.child,
    pid: state.child?.pid,
...
export function startTopstepxSmaSecondProcess(options: { accountId?: number } = {}) {
  if (state.child) {
  ...
  state.currentAccountId = options.accountId ?? (process.env.TOPSTEPX_ACCOUNT_ID ? Number(process.env.TOPSTEPX_ACCOUNT_ID) : null);
  ...
  proc.on('exit', (code, signal) => {
    ...
    appendLog(
      'stdout',
      Buffer.from(`process exited (code=${code ?? 'n/a'} signal=${signal ?? 'n/a'})`),
    );
  });
...
export function stopTopstepxSmaSecondProcess() {
...
export function flattenTopstepxSmaSecondPosition() {
  const runningChild = getChild();
  if (!runningChild) {
    return { success: false, message: 'Strategy is not running.' };
  }
