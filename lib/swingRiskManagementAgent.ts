...
🛡️ REMEMBER: Swing trades need TIME and ROOM. Don't day-trade a swing position!`;
  ...
  try {
    console.log(`[SwingRiskMgmt] 💭 Calling DeepSeek R1 for swing risk analysis...`);
    ...
    try {
      ...
      console.error(`[SwingRiskMgmt] ❌ Streaming error after ${chunkCount} chunks:`, streamError.message);
      throw new Error(`Streaming failed: ${streamError.message}`);
    }

    if (!fullContent) {
      console.log(`[SwingRiskMgmt] ⚠️ Empty response after ${chunkCount} chunks`);
      throw new Error('No response from swing risk management agent');
    }

    console.log(`[SwingRiskMgmt] 📝 Received response (${fullContent.length} chars, ${reasoningContent.length} reasoning chars)`);
