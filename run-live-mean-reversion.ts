...
const CONFIG = {
  symbols: (process.env.MR_SYMBOLS || 'SPY,GLD,TSLA,NVDA')
    ...
    .map(s => s.trim())
    .filter(Boolean),
  optionContracts: Number(process.env.MR_OPTION_CONTRACTS || '2'),
  stockShares: Number(process.env.MR_STOCK_SHARES || '0'),
  pollIntervalMs: Number(process.env.MR_POLL_MS || '15000'),
  minuteBackfill: Number(process.env.MR_MINUTE_BACKFILL || '600'),
  twelveDataApiKey: process.env.TWELVE_DATA_API_KEY || '',
  twelveDataUrl: process.env.TWELVE_DATA_WS_URL || '',
};

const TRADE_LOG_FILE = process.env.MR_TRADE_LOG || './logs/mean-reversion-trades.jsonl';
const TRADE_LOG_DIR = path.dirname(TRADE_LOG_FILE);

const trades = new Map<string, ActiveTrade>();
const lastProcessed15mBar = new Map<string, string>();
...
const TRADE_MODE = (process.env.MR_TRADE_MODE || 'option').toLowerCase();
...
async function waitForStockFill(
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (order.status === 'filled') {
      return { filledQty, avgPrice, status: order.status as OrderFillResult['status'] };
    }
    if (
  ...
  if (cancelOnTimeout) {
    ...
    return {
      ...
      status: order.status as OrderFillResult['status'],
    };
  }
...
async function getOptionChainWithFallback(
  ...
  if (cached && cached.contracts.length > 0) {
    ...
    log(
      symbol,
      `Cached option chain for ${expiration} is stale (updated ${cached.updatedAt}); ignoring`,
    );
  }
...
async function attemptEntry(symbol: string, signal: MeanReversionSignal, referencePrice: number) {
  ...
  if (USE_STOCK_TRADING) {
    ...
    const action = side === 'buy' ? 'BUY' : 'SELL SHORT';
    log(
      symbol,
      `Attempting ${action} stock entry @ ${referencePrice.toFixed(2)} for ${shares} shares`,
    );

    const fill = await submitStockMarketOrder(symbol, side, shares);
    ...
    const trade: ActiveTrade = {
      ...
      costBasis: entryPrice * fill.filledQty,
      realizedPnL: 0,
      fills: [
        {
          type: 'entry',
    ...
    log(
      symbol,
      `Entered ${direction.toUpperCase()} stock position ${symbol} @ ${entryPrice.toFixed(
        2,
...
async function scaleTrade(trade: ActiveTrade) {
  ...
  if (trade.instrument === 'stock') {
    ...
    if (trade.direction === 'long') {
      if (trade.target) {
        trade.stopLoss = trade.target * 0.99;
...
async function handleRealtimeTick(symbol: string, snapshot: RealtimePriceSnapshot) {
  ...
  try {
    ...
    const realtimeMid = trade.signal.bbMiddle;
    if (
      typeof realtimeMid === 'number' &&
      typeof trade.target === 'number' &&
      realtimeMid !== trade.target
    ) {
      trade.target = realtimeMid;
    }
...
async function processActiveTrade(trade: ActiveTrade) {
  ...
  try {
    let minuteBars = twelveDataFeed.getRecentBars(trade.symbol, '1Min', 3);

    if (minuteBars.length === 0) {
      await twelveDataFeed.syncSymbol(trade.symbol, true);
      minuteBars = twelveDataFeed.getRecentBars(trade.symbol, '1Min', 3);
    }
    ...
    if (trade.lastMinuteBar === latest.t) {
      return;
    }
    trade.lastMinuteBar = latest.t;
...
async function processSymbol(symbol: string) {
  ...
  let bars = twelveDataFeed.getRecentBars(symbol, '15Min', 40).filter(bar => !bar.partial);

  if (bars.length < 20) {
    log(symbol, 'Insufficient 15m bars for signal');
    return;
  }
  ...
  if (Number.isFinite(latestBarTime)) {
    ...
    if (ageMs > 17 * 60_000) {
      log(
        symbol,
        `Latest 15m bar stale (${(ageMs / 60000).toFixed(
          1,
        )} min old); forcing Twelve Data resync`,
      );
      await twelveDataFeed.syncSymbol(symbol, true);
      const refreshed = twelveDataFeed.getRecentBars(symbol, '15Min', 40).filter(bar => !bar.partial);
  ...
  const prevBar = lastProcessed15mBar.get(symbol);

  if (prevBar === barId) {
    return;
  }

  lastProcessed15mBar.set(symbol, barId);
  ...
  if (signal.direction === 'none') {
    log(symbol, 'No mean reversion signal on latest bar');
    return;
  }
  ...
  if (activeTrade) {
    if (typeof signal.stopLoss === 'number') {
      activeTrade.stopLoss = signal.stopLoss;
    }
    if (typeof signal.target === 'number') {
      activeTrade.target = signal.target;
    }
    activeTrade.signal = signal;
    log(
      symbol,
      `Active trade refreshed from latest bar; stop ${activeTrade.stopLoss?.toFixed(2) ?? 'n/a'}, target ${activeTrade.target?.toFixed(2) ?? 'n/a'}`,
    );
    return;
  }
...
async function main() {
  console.log('🚀 Starting live mean reversion manager');
