...
interface StatusResponse {
  ...
  trades: { openTrade: StrategyTrade | null; closedTrades: StrategyTrade[] };
  symbol: string;
...
export default function TopstepxSmaSecondDashboard() {
  ...
  const unrealizedPnl = useMemo(() => {
    ...
    return (price - openTrade.entryPrice) * direction * multiplier * Math.max(openTrade.contracts, 1);
  }, [openTrade, price, multiplier]);

  const loadStatus = useCallback(async () => {
    try {
      const response = await fetch('/api/topstepx/sma-second/status');
      if (!response.ok) {
  ...
  const loadPrice = useCallback(async () => {
    try {
      const response = await fetch('/api/topstepx/sma-second/price');
  ...
  const runAction = useCallback(async (action: 'start' | 'stop' | 'flatten') => {
    ...
    try {
      ...
      const response = await fetch('/api/topstepx/sma-second/control', {
        method: 'POST',
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch('/api/topstepx/sma-second/bars?seconds=3600');
        if (!response.ok) return;
        const payload = await response.json();
  ...
  useEffect(() => {
    const source = new EventSource('/api/topstepx/sma-second/stream');
    source.onmessage = event => {
      try {
        const payload = JSON.parse(event.data);
      ...
      return (
        <>
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold">{openTrade.contracts}</span>
          </div>
          <div className="flex justify-between">
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-6 space-y-6">
      <header className="flex flex-col gap-2">
        <h1 className="text-3xl font-semibold">TopstepX SMA (1s) Dashboard</h1>
        <p className="text-slate-400">
          Live controls and telemetry for the SMA + RSI + ADX scalper ({symbol})
        </p>
      ...
      <section className="bg-slate-800 rounded-xl p-4 shadow-xl relative">
        ...
        <div className="bg-slate-800 rounded-xl p-4 shadow space-y-2">
          ...
          <div className="grid grid-cols-3 gap-3 pt-3 mt-3 border-t border-slate-700 text-center text-xs uppercase tracking-wide text-slate-400">
            <div>
